...
Notes
To keep the things simple, we'll consider that anything with a number in front of it is a drink: "1 bear" =>
 "1 glass of water" or "1 chainsaw and 2 pools" => "3 glasses of water"
The number in front of each drink lies in range [1; 9]
 */

function hydrate(s) {
    s = s.replace(/\D/g, '').split('').reduce((acc, el) => acc + +el, 0);
    return s === 1 ? '1 glass of water' : `${s} glasses of water`;
}